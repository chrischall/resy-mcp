...
- `resy_book` flow: `findSlotsAtVenue` → `GET /3/details?config_id=...` for the `book_token` → `POST /3/book` with `struct_payment_method`. Default payment method is resolved from `/2/user` if `payment_method_id` is omitted. It books only the exact slot a preview showed: `desired_time` + `slot_type` (several seatings share a time, each with its own fees) + `terms_token` (`bookingTermsToken()`, a fingerprint of the slot type and the raw `/3/details` cancellation/payment blocks). Any mismatch re-previews instead of booking, with or without a `confirmToken`.
- **Confirmations** (`resy_book`, `resy_cancel`): mcp-utils `requireConfirmationWithFallback` + `confirmationFromEnv`. A client that supports elicitation gets a prompt; one that cannot be prompted gets the two-step token flow (phase 1 returns `status: "confirmation-required"`, the preview and a `confirmToken`; phase 2 repeats the call with it), governed by `MCP_CONFIRM_MODE` / `MCP_CONFIRM_TTL_SECONDS` / `MCP_CONFIRM_SECRET`. The token binds a payload rebuilt from a fresh read on every call (`resy_book`: venue, date, party size, slot time + type, terms token, payment id, `allow_duplicate`; `resy_cancel`: the reservation looked up by `resy_token`), so a change between the phases is refused as `DRAFT_CHANGED`. The pinned-slot and duplicate guards run exactly as before around the gate.

...
resy() {
  ...
  curl -s -X "$method" "https://api.resy.com${path}" \
    ...
    ${body:+-H 'Content-Type: application/x-www-form-urlencoded' --data "$body"}
}
```
...
resy POST /3/venuesearch/search "struct_data=$(jq -sRr @uri <<<"$STRUCT")" \
...
resy POST /3/book "book_token=<BOOK_TOKEN>&struct_payment_method=$(jq -nc --argjson id 456 '{id:$id}' | jq -sRr @uri)&source_id=resy.com-venue-details" \
  | jq '{resy_token, reservation_id, time_slot, num_seats}'

# cancel (mutates — real cancellation)
resy POST /3/cancel "resy_token=<RESY_TOKEN_ID>" | jq .
```
...
resy POST /3/user/favorites "venue_id=123&favorite=1"
resy POST /3/user/favorites "venue_id=123&favorite=0"
```
...
resy POST /2/notify "venue_id=123&day=2026-08-01&num_seats=2&time_preferred_start=18:00:00&time_preferred_end=21:00:00&service_type_id=2"
